...
/* check the signal type and add it */
...
static int step_bp(pid_t pid)
{
    ...
    else if(bp.set == 1)
    {
        ...
        if(get_regs(&regs2, pid) == -1)
            return -1;

        if(bp.addr == regs2.rip)
        {
    ...
    struct user_regs_struct regs3;
    if(get_regs(&regs3, pid) == -1)
        return -1;
    printf("stepped\n");
    regs_value(pid, "rip");
