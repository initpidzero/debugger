...
'''
This function compares results from text file dbg_log.txt and toy.txt
with Parameter: expected1. If dbg_log.txt has match with expected1 string
with Parameter: expected2. If toy.txt has exact match with expected2 string
it is a pass 
'''
...
'''
This function compares results from text file dbg_log.txt for two diffrent strings
...
'''
This function calls debuger binary for testing  
Parameters: argvs: list of string to be sent to debugger
'''
def test(argvs):
...
'''
This function runs one test after the other 
in order.
This function uses various combination of above function to run these tests
'''
def tests():
